...
/**
 * PingHTTPSession3 tests the servers ability to manage and persist large HTTPSession data objects.
 * The servlet creates the large custom java object {@link PingSession3Object}. This large session
 * object is retrieved and stored to the session on each user request. The default settings result
 * in approx 2024 bits being retrieved and stored upon each request.
 */
public class PingSession3 extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// this is a general try/catch block. The catch block at the end of this will forward the
		// responce
		...
		try {
			...
			// NUM_OBJECTS sets the size of the array to allocate and thus set the size in KBytes of
			// the session object
			// NUM_OBJECTS can be initialized by the servlet
			// Here we check for the request parameter to change the size and invalidate the session
			// if it exists
			// NOTE: Current user sessions will remain the same (i.e. when NUM_OBJECTS is changed,
			// all user thread must be restarted
			...
			outputBuffer.append("<B>Hit Count: ").append(hitCount).append(
					"<BR>Session object updated. Session Object size = " + num_bytes + " bytes </B></body></html>");
