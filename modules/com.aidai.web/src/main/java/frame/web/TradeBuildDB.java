...
import com.aidai.utils.Log;
import com.aidai.utils.TradeConfig;

import frame.model.AccountDataBean;
import framework.generic.dao.TradeDBManager;
import framework.generic.dao.TradeServiceUtilities;
import framework.generic.dao.TradeServices;
import framework.generic.dao.TradeServicesManager;

/**
 * TradeBuildDB uses operations provided by the TradeApplication to (a) create the Database tables
 * (b) populate a AriesTrader database without creating the tables. Specifically, a new AriesTrader
 * User population is created using UserIDs of the form "uid:xxx" where xxx is a sequential number
 * (e.g. uid:0, uid:1, etc.). New stocks are also created of the form "s:xxx", again where xxx
...
public class TradeBuildDB {
	...
	/**
	 * Re-create the AriesTrader db tables and populate them OR just populate a AriesTrader DB,
	 * logging to the provided output stream
	 */
	public TradeBuildDB(java.io.PrintWriter out, boolean createTables) throws Exception {
		...
		if (createTables) {
			...
			try {
				...
				if (dbProductName.startsWith("DB2/")) { // if db is DB2
					fileLocation = File.separatorChar + "dbscripts" + File.separatorChar + "db2" + File.separatorChar
							+ "Table.ddl";
				} else if (dbProductName.startsWith("Apache Derby")) { // if db is Derby
					fileLocation = File.separatorChar + "dbscripts" + File.separatorChar + "derby" + File.separatorChar
							+ "Table.ddl";
				} else if (dbProductName.startsWith("Oracle")) { // if the Db is Oracle
					fileLocation = File.separatorChar + "dbscripts" + File.separatorChar + "oracle" + File.separatorChar
							+ "Table.ddl";
				} else { // Unsupported "Other" Database
					fileLocation = File.separatorChar + "dbscripts" + File.separatorChar + "other" + File.separatorChar
							+ "Table.ddl";
			...
			try {
				...
				Log.error(e, "TradeBuildDB: Unable to drop and recreate AriesTrader Db Tables, "
		...
		for (int i = 0; i < TradeConfig.getMAX_QUOTES(); i++) {
			...
			try {
				tradeServices.createQuote(symbol, companyName, new java.math.BigDecimal(TradeConfig.rndPrice()));
				if (i % 10 == 0) {
					out.print("....." + symbol);
					if (i % 100 == 0) {
						out.println(" -<BR>");
		...
		for (int i = 0; i < TradeConfig.getMAX_USERS(); i++) {
			...
			try {
				AccountDataBean accountData = tradeServices.register(userID, "xxx", fullname, address, email,
						creditcard, new BigDecimal(initialBalance));
				if (accountData != null) {
					if (i % 50 == 0) {
	...
	public Object[] parseDDLToBuffer(URL ddlFile) throws Exception {
		...
		ArrayList sqlBuffer = new ArrayList(30); // initial capacity 30 assuming we have 30 ddl-sql
													// statements to read
		try {
			if (Log.doTrace())
			...
			while ((s = br.readLine()) != null) {
				s = s.trim();
				if ((s.length() != 0) && (s.charAt(0) != '#')) // Empty lines or lines starting with
																// "#" are ignored
				{
					sql = sql + " " + s;
