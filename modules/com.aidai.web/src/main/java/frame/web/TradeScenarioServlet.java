 ...
 * for a randomly chosen user on each access to the URL. Test this servlet by clicking Trade
 * Scenario and hit "Reload" on your browser to step through a Trade Scenario. To benchmark using
 * this URL aim your favorite web load generator (such as AKStress) at the Trade Scenario URL and
 * fire away.
 */
public class TradeScenarioServlet extends HttpServlet {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// These operations require the user to be logged in. Verify the user and if not logged
			// in
		...
		switch (action) {
			...
			case 'u': // update account profile
				...
				dispPath = tasPathPrefix + "update_profile&fullname=" + fullName + "&password=" + password
						+ "&cpassword=" + password + "&address=" + address + "&email=" + email + "&creditcard="
						+ creditcard;
				ctx.getRequestDispatcher(dispPath).include(req, resp);
				break;
			case 'h': // home
			...
			case 's': // sell
				...
				if (numHoldings > 0) {
					...
					if (foundHoldingToSell)
						break;
					if (Log.doTrace())
						Log.trace("TradeScenario: No holding to sell -switch to buy -- userID = " + userID
								+ "  Collection count = " + numHoldings);
				}
				...
				/*
				 * Trade 2.037: Added sell_deficit counter to maintain correct buy/sell mix. When a
				 * users portfolio is reduced to 0 holdings, a buy is requested instead of a sell.
				 * This throws off the buy/sell mix by 1. This results in unwanted holding table
				 * growth To fix this we increment a sell deficit counter to maintain the correct
				 * ratio in getScenarioAction The 'z' action from getScenario denotes that this is a
				 * sell action that was switched from a buy to reduce a sellDeficit
				 */
