...
import frame.model.RunStatsDataBean;
 ...
 * {@link org.apache.aries.samples.ariestrader.web.TradeConfig} JavaBean holding all configuration
 * and runtime parameters for the Trade application
 */
public class TradeConfigServlet extends HttpServlet {
	...
	/**
	 * Create the TradeConfig bean and pass it the config.jsp page to display the current Trade
	 * runtime configuration Creation date: (2/8/2000 3:43:59 PM)
	 */
	...
	/**
	 * Update Trade runtime configuration parameters Creation date: (2/8/2000 3:44:24 PM)
	 */
	void doConfigUpdate(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		currentConfigStr += "\t\tRunTimeMode:\t\t"
				+ TradeConfig.runTimeModeNames[TradeConfig.getRunTimeMode().ordinal()] + "\n";
		...
		if (jpaLayerStr != null) {
			try {
				...
				if ((i >= 0) && (i < TradeConfig.jpaLayerNames.length)) // Input validation
					TradeConfig.jpaLayer = i;
			} catch (Exception e) {
				Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
						"trying to set JPALayer to " + jpaLayerStr, "reverting to current value");
			} // If the value is bad, simply revert to current
		}
		...
		if (orderProcessingModeStr != null) {
			try {
				...
				if ((i >= 0) && (i < TradeConfig.orderProcessingModeNames.length)) // Input
																					// validation
		...
		if (accessModeStr != null) {
			try {
				...
				if ((i >= 0) && (i < TradeConfig.accessModeNames.length) && (i != TradeConfig.getAccessMode())) // Input
																												// validation
		...
		if (workloadMixStr != null) {
			try {
				...
				if ((i >= 0) && (i < TradeConfig.workloadMixNames.length)) // Input validation
					TradeConfig.workloadMix = i;
			} catch (Exception e) {
				Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
						"trying to set workloadMix to " + workloadMixStr, "reverting to current value");
			} // If the value is bad, simply revert to current
		}
		...
		if (cachingTypeStr != null) {
			try {
				...
				Log.error(e, "TradeConfigServlet.doConfigUpdate(..): minor exception caught",
						"trying to set CachingType to " + cachingTypeStr, "reverting to current value");
			} // If the value is bad, simply revert to current
		}
