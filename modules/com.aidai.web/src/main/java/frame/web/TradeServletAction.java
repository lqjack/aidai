...
import frame.model.AccountProfileDataBean;
import frame.model.HoldingDataBean;
import frame.model.OrderDataBean;
import frame.model.QuoteDataBean;
import framework.generic.dao.TradeServices;

/**
 * TradeServletAction provides servlet specific client side access to each of the Trade brokerage
 * user operations. These include login, logout, buy, sell, getQuote, etc. TradeServletAction
 * manages a web interface to Trade handling HttpRequests/HttpResponse objects and forwarding
 * results to the appropriate JSP page for the web interface. TradeServletAction invokes
 * {@link TradeServices} methods to actually perform each trading operation.
 */
public class TradeServletAction {
	...
	/**
	 * Display User Profile information such as address, email, etc. for the given Trader Dispatch
	 * to the Trade Account JSP for display
	...
	void doAccount(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String results)
		...
		try {
			...
			ArrayList orderDataBeans = (TradeConfig.getLongRun() ? new ArrayList()
					: (ArrayList) tradeServices.getOrders(userID));
	 ...
	 * the Trade Account JSP for display If any in put is incorrect revert back to the account page
	 * w/ an appropriate message
	...
	void doBuy(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String symbol,
		...
		String results = "";
		try {
			OrderDataBean orderData = tradeServices.buy(userID, symbol, new Double(quantity).doubleValue(),
	...
	/**
	 * Register a new trader given the provided user Profile information such as address, email,
	 * etc. Dispatch to the Trade Home JSP for display
	...
	void doRegister(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String passwd,
		...
		try {
			...
			if ((passwd.equals(cpasswd)) && (passwd.length() >= 1)) {
				AccountDataBean accountData = tradeServices.register(userID, passwd, fullname, address, email, ccn,
	...
	void doSell(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, Integer holdingID)
		...
		String results = "";
		try {
			OrderDataBean orderData = tradeServices.sell(userID, holdingID, TradeConfig.orderProcessingMode);
			req.setAttribute("orderData", orderData);
			req.setAttribute("results", results);
		} catch (java.lang.IllegalArgumentException e) { // this is a user
															// error so I will
